...
class Pilpres2019KompascomSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        articles_top_5 = response.css("div.row.article__wrap__grid--flex.col-offset-fluid.mt2 > div> div.article__grid")

        if articles_top_5:
            for article in articles_top_5:
                url_selector = article.css("div.article__box > h3.article__title > a::attr(href)")
                if not url_selector:
                    continue
                url = url_selector.extract_first()
                print url

                info_selectors = article.css("div.article__box > div.article__date::text")
        ...
        if articles:
            for article in articles:
                ...
                if self.media['last_crawl_at'] >= published_at:
                    is_no_update = True
                    break #might be executed even before the valid one finish.
